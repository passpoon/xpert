...
//            from  = DateUtil.convertDateFormat(searchVo.getStartDttm(), SearchVo.DATE_FORMAT, TIMESTAMP_T_PATTERN);
    ...
    public PagingResVo<LogResDto> pageAppLog(List<Integer> appResourceIds, PagingReqVo page, SearchReqVo search) {

//        ServerResource serverResource = serverResourceRepository.findById(serverResourceId);
        List<AppResource> appResources = appResourceRepository.findAllByIdIn(appResourceIds);
        //Collection<ServerResource> serverResources = serverResourceRepository.findAllByIdIn(serverResourceIds);

        if (appResources == null) {
            throw new BusinessException("noSearchApp", appResources + "");
        }
...
//        }
        ...
        filters.must(QueryBuilders.termsQuery("host", hostNames));
        filters.must(QueryBuilders.termsQuery("program", programs));
...
//            from  = DateUtil.convertDateFormat(searchVo.getStartDttm(), SearchVo.DATE_FORMAT, TIMESTAMP_T_PATTERN);
    ...
    public PagingResVo<LogResDto> pageAppLog(Integer appResourceId, PagingReqVo page, SearchReqVo search) {
        List<Integer> appResourceIds = new ArrayList<Integer>();
        appResourceIds.add(appResourceId);

        return pageAppLog(appResourceIds, page, search);
    }
